...
class JSONParserTests: XCTestCase {
    ...
    func testThatParserUnderstandsNumbers() {
        for (s, shouldBeInt) in [
            ...
            XCTAssertEqual(try! JSONFromString(s).int(), shouldBeInt)
        }
        ...
        for (s, shouldBeDouble) in [
            ...
            XCTAssertEqualWithAccuracy(try! JSONFromString(s).double(), shouldBeDouble, accuracy: DBL_EPSILON)
        }
    }
